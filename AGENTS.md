...
Refinement added 2026-09-17: importing from `llm/` is **not** the same as calling the model. `llamadas_al_modelo` / `reiniciar_contador` are observability — `orquestacion/agente_flujo.py` uses them to report how many calls happened, and one of its runs measures zero. Only `construir_cliente` / `kwargs_json` count. **Settled 2026-09-17, sooner than expected:** once the conversation moved *inside* the workflow, running it does call the model, so both the orchestrator and the entry point took the prefix — `orquestacion/agente_flujo.py` and `agente_chat.py`. The rule now has exactly one exception (the `llm/` package) and no others: a convention with exceptions stops being checkable. `agente_chat.py` also matches what `poc-agentes` called its chat entry point.
